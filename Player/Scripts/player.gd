...
func update_facing_direction():
	var positive_position = Vector2(17, slash.position.y)
	var negative_position = Vector2(-21, slash.position.y)
	
	if direction > 0:
		main_character.set_deferred("flip_h", false)
		slash.set_deferred("position", positive_position)
	elif direction < 0:
		main_character.set_deferred("flip_h", true)
		slash.set_deferred("position", negative_position)
